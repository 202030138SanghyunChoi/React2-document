...
> 클라이언트 사이드 컴포넌트와 Redux 에 대해 작성합니다.
### 💻 클라이언트 사이드 컴포넌트
useEffect, useState 등 클라이언트에서만 사용할 수 있는 컴포넌트들을 클라이언트 사이드 컴포넌트라고 합니다.

Next.js 는 기본적으로 React 프레임워크를 서버 사이드 렌더링을 하기 위해 설계되었으므로 서버에서 렌더링되도록 지향되어 설계됩니다.

따라서 클라이언트 사이드에서 렌더링을 해야하는 클라이언트 사이드 컴포넌트를 사용할 때에는 ```"use client"```를 통해 클라이언트 사이드에서 렌더린한다고 명시해야 합니다.

만약 layout.js 에서 ```"use client"```를 사용하게 된다면 전체 페이지가 렌더링 되기 때문에 따로 컴포넌트를 만들어 layout.js 에 import 하는 방식으로 클라리언트 사이드 컴포넌트를 사용하는 것을 권장합니다.



### 📁 Redux 디렉토리 구조
Next.js App 프로젝트에서는 통상적으로 다음과 같은 디렉토리 구조를 사용합니다.

* src - src 를 이용하여 정적 자원을 제외한 대부분의 디렉토리 및 파일들을 이 디렉토리 밑에서 관리합니다.
* app - Next.js 의 페이지 라우팅을 수행하는 디렉토리입니다.
* components - 재사용이 가능한 통상적인 컴포넌트들을 관리하는 디렉토리입니다.
* context - Context API 에서 사용하는 context 들을 관리하는 디렉토리입니다.
* features - Redux 에서 특정한 목적을 위해 만든 기능별 컴포넌트들을 관리하는 디렉토리입니다. 통상적으로 재사용이 불가능합니다.
* store - Redux 에서 만든 features 를 등록하여 사용할 수 있게 하는 store 와 Provider 컴포넌트들을 관리하는 디렉토리입니다.
* styles - CSS, SASS 등 스타일 파일들을 관리하는 디렉토리입니다.



### ⚔ Redux vs Context API
Context API
* React 에서 기본으로 제공하므로 외부 라이브러리를 추가로 설치하지 않아도 됩니다.
* 주로 전역 상태를 관리할 때 사용합니다.
* Context 객체와 Provider 를 사용하여 Provider 내에서 전역 상태를 관리할 수 있습니다.
* 컴폰넌트 트리에서 깊이 제한이 없어 Props Drilling 의 부작용을 해결할 수 있습니다.
* 복잡한 상태의 컴포넌트 관계에서는 관리가 어렵습니다.
* 넓은 범위의 컴포넌트 관계를 설정한다면 성능에 영향을 미칠 수 있습니다.

Redux
* 전역 상태 관리를 위한 독립적인 state 관리 라이브러리입니다.
* 상태 변경을 예측할 수 있습니다.
* 전역 state 관리를 상대적으로 더 구조적으로 다룰 수 있습니다.
* store, reducer, action 등의 개념을 사용하여 state dispatch 를 관리할 수 있습니다.
* 미들 웨어를 지원하여 비동기 로직을 쉽게 다룰 수 있습니다.
* 디버깅 도구가 있어 상태 변화와 디버깅이 용이합니다.



### ⚛ Redux 사용 - Slice
<div align="center">
    <img src="imageREADME/imageFeatureFile.png" width="400">
</div>

전역으로 사용할 기능 컴포넌트를 features 디렉토리에 기능별로 디렉토리를 만든 후 slice 파일을 생성합니다.
...
```createSlice``` 를 통해 slice 객체를 생성합니다.

```name:``` 은 각 slice 객체들을 구별하는 고유 식별자입니다.

```initialState:``` 는 전역 변수의 초기값을 결정합니다.

```reducers:``` 는 action 할 함수들을 정의합니다.

```export const { increment, decrement } = counterSlice.actions;``` 를 통해 외부에서 action 함수로 사용할 수 있게 설정합니다.

```export default counterSlice.reducer;``` 를 통해 Slice 객체의 reducer 만 외부에서 사용할 수 있게 export default 합니다.

각각의 명칭은 변경할 수 있지만 네이밍 컨베션과 같이 지키는 것을 지향합니다.



### ⚛ Redux 사용 - Store
<div align="center">
    <img src="imageREADME/imageStoreFile.png" width="400">
</div>

제작한 Slice 객체에서 추출한 Reducer 를 사용하기 위해 store 파일을 ```/store``` 디렉토리에 만듭니다.
...
configureStore() 를 통해 store 를 생성합니다.

```reducer:``` 에 Slice 의 고유식별자 이름과 추출한 reducer 를 import 함으로써 store 에 등록합니다.
